...
cpPolylineShrink(cpPolyline *line)
{
	...
	return static_cast<cpPolyline*>(cprealloc(line, cpPolylineSizeForCapacity(line->count)));
}
...
cpPolylineGrow(cpPolyline *line, int count)
{
  ...
  if(line->capacity < capacity){
    line->capacity = capacity;
		line = static_cast<cpPolyline*>(cprealloc(line, cpPolylineSizeForCapacity(capacity)));
  }
...
cpPolylineSetInit(cpPolylineSet *set)
{
	set->count = 0;
	set->capacity = 8;
	set->lines = static_cast<cpPolyline**>(cpcalloc(set->capacity, sizeof(cpPolyline)));
...
cpPolylineSetPush(cpPolylineSet *set, cpPolyline *line)
{
  ...
  if(set->count > set->capacity){
    set->capacity *= 2;
    set->lines = static_cast<cpPolyline**>(cprealloc(set->lines, set->capacity*sizeof(cpPolyline)));
  }
...
{
	int first;
	cpVect *hullVerts = static_cast<cpVect*>(alloca(count*sizeof(cpVect)));
	...
	if(hullCount != count){
		...
		if(notch.d > tol){
			...
			if(steiner_it >= 0.0){
				...
				cpVect *scratch = static_cast<cpVect*>(alloca((IMAX(sub1_count, sub2_count) + 1)*sizeof(cpVect)));
