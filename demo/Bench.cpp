...
void Bench::AddCircle(cpSpace *space, int index, cpFloat radius){
...
void Bench::AddBox(cpSpace *space, int index, cpFloat size){
...
void Bench::AddHexagon(cpSpace *space, int index, cpFloat radius){
    cpVect hexagon[6] = {};
    for(int i=0; i<6; i++){
...
cpSpace *Bench::Init_SimpleTerrainCircles_1000(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<1000; i++) AddCircle(space, i, 5.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainCircles_500(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<500; i++) AddCircle(space, i, 5.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainCircles_100(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<100; i++) AddCircle(space, i, 5.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainBoxes_1000(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<1000; i++) AddBox(space, i, 10.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainBoxes_500(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<500; i++) AddBox(space, i, 10.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainBoxes_100(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<100; i++) AddBox(space, i, 10.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainHexagons_1000(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<1000; i++) AddHexagon(space, i, 5.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainHexagons_500(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<500; i++) AddHexagon(space, i, 5.0f);

    return space;
}

cpSpace *Bench::Init_SimpleTerrainHexagons_100(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<100; i++) AddHexagon(space, i, 5.0f);
...
cpFloat Bench::RandSize(){
    return cpfpow(1.5, cpflerp(-1.5, 3.5, frand()));
}

cpSpace *Bench::Init_SimpleTerrainVCircles_200(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<200; i++) AddCircle(space, i, 5.0f*RandSize());

    return space;
}

cpSpace *Bench::Init_SimpleTerrainVBoxes_200(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<200; i++) AddBox(space, i, 8.0f*RandSize());

    return space;
}

cpSpace *Bench::Init_SimpleTerrainVHexagons_200(){
    cpSpace *space = SetupSpace_SimpleTerrain();
    for(int i=0; i<200; i++) AddHexagon(space, i, 5.0f*RandSize());
...
cpBool Bench::NoCollide_begin(cpArbiter *arb, cpSpace *space, void *data){
    abort();
