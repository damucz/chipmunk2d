...
cpSpace *Shatter::Init()
{
    ChipmunkDemo::Init();

    message = "Hold right bottom corner and tap shape to shatter it.";
...
cpVect Shatter::HashVect(uint32_t x, uint32_t y, uint32_t seed)
{
...
cpVect Shatter::WorleyPoint(int i, int j, struct WorleyContex *context)
{
...
int Shatter::ClipCell(cpShape *shape, cpVect center, int i, int j, struct WorleyContex *context, cpVect *verts, cpVect *clipped, int count)
{
...
void Shatter::ShatterCell(cpSpace *space, cpShape *shape, cpVect cell, int cell_i, int cell_j, struct WorleyContex *context)
{
...
void Shatter::ShatterShape(cpSpace *space, cpShape *shape, cpFloat cellSize, cpVect focus)
{
...
bool Shatter::ProcessTouch( uint32 id, cpVect pos, TouchState state, bool rightClick /*= false*/ )
{
    if(rightClick){
        if (state == ChipmunkDemo::TOUCH_START)
        {
            cpPointQueryInfo info;
            if(cpSpacePointQueryNearest(space, pos, 0, GRAB_FILTER, &info)){
                cpBB bb = cpShapeGetBB(info.shape);
                cpFloat cell_size = cpfmax(bb.r - bb.l, bb.t - bb.b)/5.0f;
                if(cell_size > 5.0f){
                    ShatterShape(space, (cpShape *)info.shape, cell_size, pos);
                } else {
